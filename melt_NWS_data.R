...
###########################################################################################
###########################################################################################
#---------------Check agreement of LST and NWS-------------------------------
allStationsLST <- read.csv("./LST_Station.csv", header=T)

stationsLSTNWS <- merge(allStationsLST, allAdvisories, by=c("Day", "Month", "Year", "County"), all.x=TRUE)
stationsNWS2011On <- stationsLSTNWS[which(stationsLSTNWS$Year > 2005 & stationsLSTNWS$Year < 2013),]
stationsNWS2011OnClean <- na.omit(stationsNWS2011On)

ggplot(stationsNWS2011OnClean, aes(x=factor(Month), y=Temp,fill=factor(NWSadvisory))) + geom_boxplot() + labs(x="Month", y="LST at Station (˚F)") + facet_grid(County ~ Year) + theme(legend.position="bottom")

allCountiesLST <- read.csv("./LST_County.csv", header=T)

countiesLSTNWS <- merge(allCountiesLST, allAdvisories, by=c("Day", "Month", "Year", "County"), all.x=TRUE)
countiesNWS2011On <- countiesLSTNWS[which(countiesLSTNWS$Year > 2005 & countiesLSTNWS$Year < 2013),]
countiesNWS2011OnClean <- na.omit(countiesNWS2011On)

ggplot(countiesNWS2011OnClean, aes(x=factor(Month), y=Max,fill=factor(NWSadvisory))) + geom_boxplot() + labs(x="Month", y="LST County Max (˚F)") + facet_grid(County ~ Year) + theme(legend.position="bottom")
